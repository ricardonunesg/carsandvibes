...
type SelectedOptionsByGroup = Record<string, string>; // groupId -> optionId
...
function buildSelectedOptionsFromVariant(variant: any): SelectedOptionsByGroup {
  const result: SelectedOptionsByGroup = {};
  (variant?.options ?? []).forEach((opt: any) => {
    if (opt?.groupId && opt?.id) {
      result[String(opt.groupId)] = String(opt.id);
    }
  });
  return result;
}

function findVariantBySelectedOptions(product: any, selected: SelectedOptionsByGroup): any | undefined {
  const variants = product?.variants ?? [];
  const groupIds = Object.keys(selected);

  // se ainda não tiver nada selecionado, devolve a primeira
  if (!groupIds.length) return variants[0];

  return variants.find((v: any) => {
    const opts = v?.options ?? [];
    // tem de conter todas as opções selecionadas (por groupId)
    return groupIds.every((gid) => opts.some((o: any) => String(o.groupId) === gid && String(o.id) === selected[gid]));
  });
}
...
export default component$(() => {
  ...
  // Estado base: começamos com a primeira variant
  const selectedVariantIdSignal = useSignal(productSignal.value.variants[0].id);

  // Novo: seleção por optionGroup (Color, Size, etc.)
  const selectedOptionsSig = useSignal<SelectedOptionsByGroup>(
    buildSelectedOptionsFromVariant(productSignal.value.variants[0])
  );

  // Quando selectedOptions muda, escolhe a variant correspondente
  const selectedVariantSignal = useComputed$(() => {
    const v = findVariantBySelectedOptions(productSignal.value, selectedOptionsSig.value);
    if (v?.id && v.id !== selectedVariantIdSignal.value) {
      selectedVariantIdSignal.value = v.id;
    }
    return v ?? productSignal.value.variants.find((x) => x.id === selectedVariantIdSignal.value);
  });
  ...
  const quantitySignal = useComputed$<Record<string, number>>(() => {
    ...
    (productSignal.value.variants || []).forEach((variant: Variant) => {
      const orderLine = (appState.activeOrder?.lines || []).find(
        (l: OrderLine) =>
          l.productVariant.id === variant.id && l.productVariant.product.id === productSignal.value.id
      );
  ...
  const optionGroups = productSignal.value.optionGroups ?? [];
  const hasOptionGroups = optionGroups.length > 0;

  return (
    <div>
      {selectedVariantSignal.value?.id && <span class="hidden">{selectedVariantSignal.value?.id}</span>}
      ...
      <div class="max-w-6xl mx-auto px-4 py-10">
        <div>
          ...
          <Breadcrumbs
            items={
              productSignal.value.collections[productSignal.value.collections.length - 1]?.breadcrumbs ?? []
            }
          ></Breadcrumbs>
          ...
          <div class="lg:grid lg:grid-cols-2 lg:gap-x-8 lg:items-start mt-4 md:mt-12">
            ...
            <div class="mt-10 px-4 sm:px-0 sm:mt-16 lg:mt-0">
              <div class="">
                ...
                <div class="text-base text-gray-700" dangerouslySetInnerHTML={productSignal.value.description} />
              </div>

              {/* ✅ NOVO: 1 select por OptionGroup (ex: Color + Size) */}
              {hasOptionGroups && (
                <div class="mt-6 space-y-4">
                  {optionGroups.map((group) => {
                    const gid = String(group.id);
                    const selectedOptionId = selectedOptionsSig.value[gid] ?? '';
              ...
              {/* Fallback: se não houver optionGroups mas houver variants > 1, mantém o select antigo */}
              {!hasOptionGroups && 1 < productSignal.value.variants.length && (
                <div class="mt-4">
                  ...
                  >
                    {productSignal.value.variants.map((variant) => (
                      <option key={variant.id} value={variant.id} selected={selectedVariantIdSignal.value === variant.id}>
                        {variant.name}
                      </option>
                    ))}
                  </select>
                ...
                <div class="flex sm:flex-col1 align-baseline">
                  <button
                    class={{
                      ...
                      'bg-gray-600 cursor-not-allowed': quantitySignal.value[selectedVariantIdSignal.value] > 7,
                    }}
                    onClick$={async () => {
                      if (quantitySignal.value[selectedVariantIdSignal.value] <= 7) {
                        const addItemToOrder = await addItemToOrderMutation(selectedVariantIdSignal.value, 1);
              ...
              <section class="mt-12 pt-12 border-t text-xs">
                ...
                <div class="text-gray-500 space-y-1">
                  <p>{_`Standard shipping: 3 - 5 working days. Express shipping: 1 - 3 working days.`}</p>
                  <p>{_`Shipping costs depend on delivery address and will be calculated during checkout.`}</p>
                  <p>
...
export const head: DocumentHead = ({ resolveValue, url }) => {
  ...
  return generateDocumentHead(url.href, product.name, product.description, product.featuredAsset?.preview);
};
